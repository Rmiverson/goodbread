...
const CreateRecipe = () => {
  ...
  const { isLoading: isPostingRecipe, mutate: postRecipe } = useMutation(
    async () => {
      return await apiClient.post('/recipes', {recipe: dataPackage}, {headers: {'Authorization': `Bearer ${currentUser.token}`}})
    },
    {
      onSuccess: (res) => {
        const apiResp = {
  ...
  const handleBodyTextChange = (editorState, editor) => {
    const htmlStr = editorState.read(() => $generateHtmlFromNodes(editor, null))
    setBodyText(htmlStr)
  }
